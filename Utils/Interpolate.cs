...
//	public static Vector3 GetCatmullRomPoint(IEnumerable<Vector3> catmullRom, float point)
//	{
////		CatmullRom(
//		// Find the points involved 
//		int prevIndex, nextIndex;
//		
//		Vector3[] catmullArray = catmullRom.ToArray();
//		prevIndex = Mathf.FloorToInt(catmullArray.Length * point);
//		nextIndex = Mathf.CeilToInt(catmullArray.Length * point);
//		
//		return CatmullRom(catmullArray[prevIndex], catmullArray[0],
//							catmullArray[catmullArray.Length-1], catmullArray[nextIndex],
//	}
	...
	public static Vector3[] NormalizePath(IEnumerable<Vector3> pathSource, int segments)
	{
		float distance;
		return NormalizePath(pathSource, segments, out distance);
	}
	
	public static Vector3[] NormalizePath(IEnumerable<Vector3> pathSource, int segments, out float pathLength)
	{
		List<Vector3> originalPath = new List<Vector3>();
		
		// Get the distance of the path and create a list for easy reference
		float pathDistance = 0;
		Vector3 lastPoint = Vector3.zero;
		foreach (Vector3 pathPt in pathSource)
		{
			if (lastPoint != Vector3.zero)
			{
				pathDistance += (pathPt - lastPoint).magnitude;
			}
			originalPath.Add(pathPt);
			lastPoint = pathPt;
		}
		pathLength = pathDistance;
		
		float normalDistance = pathDistance / segments;
		...
		int pathIndex = 1;
		lastPoint = originalPath[0];
		// Loop through the points of the path
		while (pathIndex < originalPath.Count)
		{
			Vector3 segmentVector = originalPath[pathIndex] - lastPoint;
			
			// If distance from last point is less than the
			// average distance we want, go to the next point
			if (segmentVector.magnitude < normalDistance)
			{
				pathIndex++;
			}
			else
			{
				// Something went wrong, just return the path
				if (newPath.Count > segments)
				{
					throw new Exception("Could not normalize segment, normal distance was " + normalDistance);
				}
				
				// More than or equals to the next point
				Vector3 nextPoint = lastPoint + (segmentVector.normalized * normalDistance);
//				Debug.Log("Segment length is " + (nextPoint - lastPoint).magnitude);
				newPath.Add(nextPoint);
				lastPoint = nextPoint;
			}
			
			// If we reach the end but new path is not the expected segment count, add the last path
			// might have missed it due to floating point errors
			if (pathIndex == originalPath.Count)
			{
				if (newPath.Count < segments)
				{
	...
	public static Vector3 PointOnNormalizedPath(Vector3[] normalizedPath, float position)
	{
		float pos = Mathf.Clamp((position * normalizedPath.Length), 0f, (normalizedPath.Length-1));
		
		int pathIndex = Mathf.FloorToInt(pos);
		if (pathIndex >= (normalizedPath.Length-1))
			return normalizedPath[normalizedPath.Length-1];
		
		float pathFraction = pos - pathIndex;
		Vector3 pathDirection = (normalizedPath[pathIndex + 1] - normalizedPath[pathIndex]);
		Vector3 finalPosition = normalizedPath[pathIndex] + (pathDirection * pathFraction);
